...
vec4 calculateSpotLight(SpotLight light, Material material, vec3 lightVec, vec3 spotDirVec, vec3 normalVec, vec3 eyeVec)
{
	...
	vec3 reflectVec = reflect(-lightVec,normalVec);
	float diffuse = max(dot(normalVec,lightVec),0.0);
	float theta = dot(spotDirVec, lightVec);
...
void main() {
	gl_FragColor =
		...
		+ calculateSpotLight(u_light3, u_material,v_light3Vec, v_light3SpotDirVec, v_normalVec, v_eyeVec);
